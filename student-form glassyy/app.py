...
def submit_form():
    try:
        ...
        required_fields = ['full_name', 'father_name', 'mother_name', 'gender', 'phone_number',
                          'father_phone_number', 'mother_phone_number', 'email', 'permanent_address',
                          'address', 'dob', 'nationality', 'reservation_category', 'matric_board',
                          'degree', 'course', 'photo', 'signature']
        for field in required_fields:
            if not form_data.get(field):
                return jsonify({'error': f'{field} is required'}), 400

        # Save to MongoDB
        result = students_collection.insert_one(form_data)
...
def get_students():
    try:
        ...
        for student in students:
            ...
            if 'photo' in student:
                student['photo'] = f"/uploads/{os.path.basename(student['photo'])}"
            if 'signature' in student:
                student['signature'] = f"/uploads/{os.path.basename(student['signature'])}"
